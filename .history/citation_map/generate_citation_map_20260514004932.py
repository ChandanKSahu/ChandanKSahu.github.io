#!/usr/bin/env python3
"""
Citation Map Generator using SerpAPI + Bing Maps.
...
Workflow:
  ...
  4. Geocode institutions via Bing Maps REST API (English country names)
...
_BING_KEY_PATH = Path("/home/chandan/API Keys/bing_maps_api.txt")

SERPAPI_KEY: str = _SERP_KEY_PATH.read_text().strip() if _SERP_KEY_PATH.exists() else ""
BING_MAPS_KEY: str = _BING_KEY_PATH.read_text().strip() if _BING_KEY_PATH.exists() else ""

if not SERPAPI_KEY:
    sys.exit(f"ERROR: SerpAPI key not found at {_SERP_KEY_PATH}")
if not BING_MAPS_KEY:
    sys.exit(f"ERROR: Bing Maps key not found at {_BING_KEY_PATH}")
...
BING_GEOCODE_URL = "http://dev.virtualearth.net/REST/v1/Locations"
...
SERP_DELAY = 0.3          # seconds between SerpAPI calls
BING_DELAY = 0.15         # seconds between Bing geocode calls
...
def fetch_citing_papers(cites_id: str, expected: int) -> list[dict]:
...
# Bing Maps geocoding
# ---------------------------------------------------------------------------
def geocode_institution_bing(name: str, cache: dict) -> dict | None:
    """Geocode an institution via Bing Maps REST API.

    Returns {lat, lon, city, country, address} with English country names.
    Retries previously-failed (None) entries from old geocoder runs.
    """
    if name in cache and cache[name] is not None:
        return cache[name]

    # Clean up the query – remove job titles, "Professor at", etc.
    ...
    try:
        ...
        resp = requests.get(BING_GEOCODE_URL, params={
            "q": query,
            "key": BING_MAPS_KEY,
            "maxResults": 1,
            "culture": "en-US",
        ...
        resources = (data.get("resourceSets", [{}])[0]
                     .get("resources", []))
        if resources:
            res = resources[0]
            point = res.get("point", {}).get("coordinates", [0, 0])
            addr = res.get("address", {})
            result = {
                "lat": point[0],
                "lon": point[1],
                "address": addr.get("formattedAddress", ""),
                "city": addr.get("locality", "") or addr.get("adminDistrict", ""),
                "country": addr.get("countryRegion", ""),
            }
            cache[name] = result
            _save_json(GEOCODE_CACHE_FILE, cache)
            return result
    except Exception as exc:
        print(f"  ⚠ Bing geocode error for '{query}': {exc}")
...
def _clean_institution_name(raw: str) -> str:
    """Strip job titles and prefixes from an affiliation string.

    e.g. 'Research Scientist @ ABB US Corporate Research Center'
         -> 'ABB US Corporate Research Center'
         'Professor, Department of CS, MIT' -> 'MIT'
    """
...
def run_pipeline(checkpoint: dict) -> dict:
    ...
    for article in tqdm(to_fetch, desc="Fetching citing papers"):
        ...
        try:
            papers = fetch_citing_papers(cites_id, expected)
...
def generate_map(checkpoint: dict, geocode_cache: dict) -> tuple[int, dict]:
    ...
    for inst_name, data in tqdm(institutions.items(), desc="Geocoding"):
        coords = geocode_institution_bing(inst_name, geocode_cache)
        if not coords:
            continue
...
def main():
    print("=" * 60)
    print("  Citation Map Generator (SerpAPI + Bing Maps)")
    print("=" * 60)
