...
def generate_map(checkpoint: dict, geocode_cache: dict) -> tuple[int, dict]:
    ...
    # Banner removed – stats are shown on the recognition page instead
...
# ---------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Citation Map Generator")
    parser.add_argument(
        "--regen", action="store_true",
        help="Skip scraping; regenerate outputs from existing checkpoint",
    )
    ...
    if args.regen:
        print("\n♻️  --regen: skipping scrape, regenerating outputs from checkpoint\n")
    else:
        if not SERPAPI_KEY:
        ...
        cached_papers = sum(
            len(v) for v in checkpoint.get("citing_papers", {}).values()
        )
