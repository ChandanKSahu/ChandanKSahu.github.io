...
def format_citations_rounded(total: int) -> str:
    """Format total citations to the highest 100x multiplier below with a plus sign.
...
def process_citations(scholar_id, checkpoint):
    ...
    # Extract h-index from the indices section
...
def main():
    ...
    print("Summary")
    print("=" * 60)
    print(f"Total citations: {data.get('total_citations', 0)} ({data.get('total_citations_display', 'N/A')})")
